  ...
  return (
    <section className="bg-surface-container-low py-24">
      <div className="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
        <h2 className="text-headline-lg text-primary mb-16 text-center">{data.title}</h2>
        <div className="relative flex flex-col md:flex-row justify-between items-stretch gap-8">
          ...
          {data.steps.map((step, index) => (
            ...
            >
              <div className="w-10 h-10 rounded-full bg-secondary text-on-primary flex items-center justify-center text-label-md mb-6 shadow-lg shadow-secondary/20">
                {step.number}
              </div>
              <h4 className="text-label-md text-primary uppercase tracking-widest mb-3">{step.title}</h4>
